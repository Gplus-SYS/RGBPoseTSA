...
class RGB_Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        sample['video_name'] = sample_video_name
        sample['imgs'] = video_1_ori
        video_w, video_h,_ = video_1_ori[0].shape
        sample['img_shape'] = (video_h, video_w)
        sample['total_frames'] = len(video_1_ori)
        ...
        if self.action_number_choosing:
            ...
            if self.split == 'train':
                ...
                target['video_name'] = target_video_name
                target['imgs'] = video_2_ori
                video_w, video_h,_ = video_2_ori[0].shape
                target['img_shape'] = (video_h, video_w)
                target['total_frames'] = len(video_2_ori)
                ...
                return self.pipeline(sample), self.pipeline(target_list)
        else:
