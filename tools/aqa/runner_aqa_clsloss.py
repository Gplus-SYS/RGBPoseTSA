...
from tools.aqa import helper_aqa_clsloss as helper
...
def train_net(args):
    ...
    # build model
    backbone, cls_head, ps_net, decoder, regressor = builder.model_builder(args)
    # build opti
    optimizer, scheduler = builder.optimizer_scheduler_builder([backbone, cls_head, ps_net, decoder, regressor], args)
    if use_gpu:
        backbone.cuda()
        cls_head.cuda()
        ps_net.cuda()
    ...
    # Hyperparameter
    start_epoch = 0
    global epoch_best_tas, pred_tious_best_5, pred_tious_best_75, epoch_best_aqa, rho_best, L2_min, RL2_min, epoch_best_cls, best_top1_acc
    epoch_best_tas = 0
    pred_tious_best_5 = 0
    ...
    # # resume ckptsQ
    # if args.resume:
    #     start_epoch, epoch_best_aqa, rho_best, L2_min, RL2_min = builder.resume_train(backbone, ps_net, decoder, regressor,
    ...
    for epoch in range(start_epoch, args.total_epochs):
        ...
        if action_number_choosing:
            for idx, (data, target) in enumerate(train_dataloader):
                helper.backbone_forward_train(backbone, cls_head, ps_net, decoder, regressor, bce, mse,
                                                    data, target, optimizer, epoch, idx+1, len(train_dataloader), 
                                                    scaler, top1_accs, pred_scores, pred_tious_5, pred_tious_75, args)
        ...
        print('[Time: %s][Training] EPOCH: %d, epoch time: %dmin%ds, tIoU_5: %.4f, tIoU_75: %.4f, correlation: %.4f, L2: %.4f, RL2: %.4f, top1_acc: %.4f'
              % (datetime.now().strftime("%Y-%m-%d %H:%M:%S"), epoch, epoch_time//60, epoch_time%60, 
                 pred_tious_mean_5, pred_tious_mean_75, rho, L2, RL2, top1_acc))
        

        validate_net(backbone, cls_head, ps_net, decoder, regressor, val_dataloader, epoch, optimizer, scheduler, args)
        helper.save_checkpoint(backbone, cls_head, ps_net, decoder, regressor, optimizer, scheduler, epoch,
                                    epoch_best_aqa, rho_best, L2_min, RL2_min, top1_acc, 'last', args)
        print('[VAL] EPOCH_BEST_CLS: %d, best top1_acc: %.4f' % (epoch_best_cls, best_top1_acc))
...
def validate_net(backbone, cls_head, ps_net, decoder, regressor, val_dataloader, epoch, optimizer, scheduler, args):
    print("Start validating epoch {} ......".format(epoch))

    global use_gpu
    global epoch_best_tas, rho_best, L2_min, RL2_min, epoch_best_aqa, pred_tious_best_5, pred_tious_best_75, epoch_best_cls, best_top1_acc
    ...
    with torch.no_grad():
        ...
        if action_number_choosing:
            for _, (data, targets) in enumerate(val_dataloader, 0):
                pred, gt_label, transits_pred, transits_label = helper.backbone_forward_test(backbone, cls_head, ps_net, decoder, regressor, data, targets, top1_accs,
        ...
        if rho > rho_best:
            ...
            helper.save_checkpoint(backbone, cls_head, ps_net, decoder, regressor,
                                        optimizer, scheduler, epoch, epoch_best_aqa,
                                        rho_best, L2_min, RL2_min, top1_acc, 'best', args)
...
def test_net(args, ckpt_path = None):
    ...
    backbone, cls_head, ps_net, decoder, regressor = builder.model_builder(args)
    if ckpt_path:
        builder.load_model_c(backbone, cls_head, ps_net, decoder, regressor, ckpt_path)
    else:
        builder.load_model_c(backbone, cls_head, ps_net, decoder, regressor, args.ckpts)
    if use_gpu:
        backbone.cuda()
        cls_head.cuda()
        ps_net.cuda()
    ...
    decoder = nn.DataParallel(decoder)
    regressor = nn.DataParallel(regressor)
    test(backbone, cls_head, ps_net, decoder, regressor, test_dataloader, args)

def test(backbone, cls_head, ps_net, decoder, regressor, test_dataloader, args):
    global use_gpu
    top1_accs = []
    true_scores = []
    pred_scores = []
    ...
    with torch.no_grad():
        ...
        if action_number_choosing:
            for _, (data, target) in enumerate(test_dataloader, 0):
                pred, gt_label, transits_pred, transits_label = helper.backbone_forward_test(backbone, cls_head, ps_net, decoder, regressor, data, target, top1_accs,
                                                                                             pred_scores, pred_tious_test_5, pred_tious_test_75, args)
                preds.extend(pred)
                gt_labels.extend(gt_label)
                transits_preds.extend(transits_pred)
    ...
    print('[Time: %s][TEST] top1_acc: %.4f' % (datetime.now().strftime("%Y-%m-%d %H:%M:%S"), top1_acc))
