...
from tools.aqa import helper_aqa_MM_early_fusion as helper
...
def train_net(args):
    ...
    # build model
    backbone, ps_net, decoder, regressor = builder.model_builder(args)
    # build opti
    optimizer, scheduler = builder.optimizer_scheduler_builder([backbone, ps_net, decoder, regressor],args)
    if use_gpu:
        backbone.cuda()
        ps_net.cuda()
        decoder.cuda()
    ...
    # resume ckptsQ
    if args.resume:
        start_epoch, epoch_best_aqa, rho_best, L2_min, RL2_min = builder.resume_train(backbone, ps_net, decoder, 
    ...
    for epoch in range(start_epoch, args.total_epochs):
        ...
        if action_number_choosing:
            for idx, (data, target) in enumerate(train_dataloader):
                helper.backbone_forward_train(backbone, ps_net, decoder, regressor, bce, mse, 
        ...
        validate_net(backbone, ps_net, decoder, regressor, val_dataloader, epoch, optimizer, scheduler, args)
        helper.save_checkpoint(backbone, ps_net, decoder, regressor, optimizer, scheduler, epoch, 
...
def validate_net(backbone, ps_net, decoder, regressor, val_dataloader, epoch, optimizer, scheduler, args):
    ...
    with torch.no_grad():
        ...
        if action_number_choosing:
            for _, (data, targets) in enumerate(val_dataloader, 0):
                transits_pred, transits_label = helper.backbone_forward_test(backbone, ps_net, decoder, regressor, data, targets, 
        ...
        if rho > rho_best:
            ...
            helper.save_checkpoint(backbone, ps_net, decoder, regressor,
...
def test_net(args, ckpt_path = None):
    ...
    backbone, ps_net, decoder, regressor = builder.model_builder(args)
    if ckpt_path:
        builder.load_model(backbone, ps_net, decoder, regressor, ckpt_path)
    else:
        builder.load_model(backbone, ps_net, decoder, regressor, args.ckpts)
    if use_gpu:
        backbone.cuda()
        ps_net.cuda()
        decoder.cuda()
    ...
    decoder = nn.DataParallel(decoder)
    regressor = nn.DataParallel(regressor)
    test(backbone, ps_net, decoder, regressor, test_dataloader, args)

def test(backbone, ps_net, decoder, regressor, test_dataloader, args):
    global use_gpu
    true_scores = []
    pred_scores = []
    transits_preds = []
    ...
    with torch.no_grad():
        ...
        if action_number_choosing:
            for _, (data, target) in enumerate(test_dataloader, 0):
                transits_pred, transits_label = helper.backbone_forward_test(backbone, ps_net, decoder, regressor, data, target, pred_scores, 
